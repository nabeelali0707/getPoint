...
# Pre-defined known coordinates for prominent areas in Karachi
KNOWN_COORDS = {
...
def get_base_coords(stop_name):
    ...
    # Fallback: random coordinate around central Karachi to start with
...
def geocode_stops(stops):
    ...
    end_coords = FAST_NU_COORDS # Destination is always FAST NU
    ...
    for i, stop in enumerate(stops):
        if i == 0:
            ...
            # Add minor random noise (jitter) to make route look realistic
...
def main():
    ...
    # --- 1. PARSE JADOON ROUTES (EXCEL) ---
    excel_path = "routes/Jadoon Routes Fall 2025.xlsx"
    if os.path.exists(excel_path):
        print("Parsing Jadoon Excel...")
        ...
        for r in range(1, sheet.max_row + 1):
            ...
            # Check for route code (e.g. "Jadoon R:2" or similar)
            if cell_1 and isinstance(cell_1, str) and ("Jadoon R:" in cell_1 or "R:" in cell_1):
                ...
                # This is the driver row, e.g. "Razaq 03177153606"
                ...
                # Time could be datetime.time object or string
                ...
                # Normalize time to "HH:MM" or "HH:MM am/pm"
    ...
    # --- 2. PARSE NADEEM ROUTES (PDF) ---
    pdf_path = "routes/Nadeem Routes Fall 2025.pdf"
    if os.path.exists(pdf_path):
        print("Parsing Nadeem PDF...")
        ...
        for p_idx, page in enumerate(reader.pages):
            ...
            # Identify route code line (e.g. "ROUTE # 1-B")
            for line in lines[:5]:
                if "ROUTE #" in line:
                    route_code = line.replace("ROUTE #", "Nadeem R:").strip()
                    break
            
            # Identify driver info
            for line in lines[:8]:
            ...
            # Extract stops after timing header
            is_timing = False
            temp_stop_name = ""
            ...
            for line in lines:
                ...
                if is_timing:
                    # Look for lines that end with a time pattern
                    # e.g., "Malir Cantt Phase-I 6:45 am" or "Fast Nu 7:45 am"
                    ...
                    if time_match:
                        ...
                        # If a line doesn't end with a time, it might be a split stop name
                        # (e.g. "Security Printing" then "Press 7:08 am")
                        # Skip page headers or other fluff
    ...
    # Save to JSON
    output_path = "apps/api/src/db/seeds/routes_data.json"
    os.makedirs(os.path.dirname(output_path), exist_ok=True)
    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(routes, f, indent=2, ensure_ascii=False)
        
    print(f"Successfully parsed and geocoded {len(routes)} routes! Written to {output_path}")
